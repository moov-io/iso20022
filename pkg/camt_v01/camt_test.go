...
func TestDocumentCamt10100101(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t,
		`{"XMLName":{"Space":"","Local":""},"CretLmt":{"MsgHdr":{"MsgId":"MsgId"}}}`,
		string(buf))

	buf, err = xml.Marshal(&sample)
	assert.Nil(t, err)
	assert.Equal(t,
		`<DocumentCamt10100101><CretLmt><MsgHdr><MsgId>MsgId</MsgId></MsgHdr></CretLmt></DocumentCamt10100101>`,
		string(buf))
}
...
func TestDocumentCamt10200101(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t,
		`{"XMLName":{"Space":"","Local":""},"CretStgOrdr":{"MsgHdr":{"MsgId":"MsgId"},"StgOrdrId":{"Acct":{"Id":{"IBAN":"AA000130","Othr":{"Id":"Id"}}}},"ValSet":{}}}`,
		string(buf))

	buf, err = xml.Marshal(&sample)
	assert.Nil(t, err)
	assert.Equal(t,
		`<DocumentCamt10200101><CretStgOrdr><MsgHdr><MsgId>MsgId</MsgId></MsgHdr><StgOrdrId><Acct><Id><IBAN>AA000130</IBAN><Othr><Id>Id</Id></Othr></Id></Acct></StgOrdrId><ValSet></ValSet></CretStgOrdr></DocumentCamt10200101>`,
		string(buf))
}
...
func TestDocumentCamt10300101(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t,
		`{"XMLName":{"Space":"","Local":""},"CretRsvatn":{"MsgHdr":{"MsgId":"MsgId"},"RsvatnId":{"Tp":{"Cd":"BLKD","Prtry":"Prtry"}},"ValSet":{"Amt":{"AmtWthtCcy":0,"AmtWthCcy":{"Value":0,"Ccy":"AAA"}}}}}`,
		string(buf))

	buf, err = xml.Marshal(&sample)
	assert.Nil(t, err)
	assert.Equal(t,
		`<DocumentCamt10300101><CretRsvatn><MsgHdr><MsgId>MsgId</MsgId></MsgHdr><RsvatnId><Tp><Cd>BLKD</Cd><Prtry>Prtry</Prtry></Tp></RsvatnId><ValSet><Amt><AmtWthtCcy>0</AmtWthtCcy><AmtWthCcy Ccy="AAA">0</AmtWthCcy></Amt></ValSet></CretRsvatn></DocumentCamt10300101>`,
		string(buf))
}
...
func TestDocumentCamt10400101(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t,
		`{"XMLName":{"Space":"","Local":""},"CretMmb":{"MsgHdr":{"MsgId":"MsgId"},"MmbId":{"BICFI":"0000AA00111","ClrSysMmbId":{"MmbId":"MmbId"},"Othr":{"Id":"MmbId"}},"ValSet":{}}}`,
		string(buf))

	buf, err = xml.Marshal(&sample)
	assert.Nil(t, err)
	assert.Equal(t,
		`<DocumentCamt10400101><CretMmb><MsgHdr><MsgId>MsgId</MsgId></MsgHdr><MmbId><BICFI>0000AA00111</BICFI><ClrSysMmbId><MmbId>MmbId</MmbId></ClrSysMmbId><Othr><Id>MmbId</Id></Othr></MmbId><ValSet></ValSet></CretMmb></DocumentCamt10400101>`,
		string(buf))
}
