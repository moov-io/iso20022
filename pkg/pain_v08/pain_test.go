...
func TestDocumentPain01400108(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, string(buf), `{"CdtrPmtActvtnReqStsRpt":{"GrpHdr":{"MsgId":"MsgId","CreDtTm":"2014-11-12T11:45:26.371","InitgPty":{},"DbtrAgt":{"FinInstnId":{}},"CdtrAgt":{"FinInstnId":{}}},"OrgnlGrpInfAndSts":{"OrgnlMsgId":"OrgnlMsgId","OrgnlMsgNmId":"OrgnlMsgNmId","OrgnlCreDtTm":"2014-11-12T11:45:26.371","OrgnlNbOfTxs":"001","GrpSts":"GRP"}}}`)

	buf, err = xml.Marshal(&sample)
	assert.Nil(t, err)
	assert.Equal(t, string(buf), `<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.014.001.08" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><CdtrPmtActvtnReqStsRpt xmlns="urn:iso:std:iso:20022:tech:xsd:pain.014.001.08"><GrpHdr><MsgId>MsgId</MsgId><CreDtTm>2014-11-12T11:45:26.371</CreDtTm><InitgPty></InitgPty><DbtrAgt><FinInstnId></FinInstnId></DbtrAgt><CdtrAgt><FinInstnId></FinInstnId></CdtrAgt></GrpHdr><OrgnlGrpInfAndSts><OrgnlMsgId>OrgnlMsgId</OrgnlMsgId><OrgnlMsgNmId>OrgnlMsgNmId</OrgnlMsgNmId><OrgnlCreDtTm>2014-11-12T11:45:26.371</OrgnlCreDtTm><OrgnlNbOfTxs>001</OrgnlNbOfTxs><GrpSts>GRP</GrpSts></OrgnlGrpInfAndSts></CdtrPmtActvtnReqStsRpt></Document>`)
}
...
func TestDocumentPain01300108(t *testing.T) {
	...
	buf, err := json.Marshal(&sample)
	assert.Nil(t, err)
	assert.Equal(t, string(buf), `{"CdtrPmtActvtnReq":{"GrpHdr":{"MsgId":"MsgId","CreDtTm":"2014-11-12T11:45:26.371","NbOfTxs":"001","InitgPty":{}}}}`)

	buf, err = xml.Marshal(&sample)
	assert.Nil(t, err)
	assert.Equal(t, string(buf), `<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.013.001.08" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><CdtrPmtActvtnReq xmlns="urn:iso:std:iso:20022:tech:xsd:pain.013.001.08"><GrpHdr><MsgId>MsgId</MsgId><CreDtTm>2014-11-12T11:45:26.371</CreDtTm><NbOfTxs>001</NbOfTxs><InitgPty></InitgPty></GrpHdr></CdtrPmtActvtnReq></Document>`)
}
